      ...
      <div className="container-aurura grid lg:grid-cols-2 gap-16 items-start">
        ...
        <Reveal delay={0.1} className="bg-white border border-hairline p-7 md:p-10">
          ...
          {sent ? (
            ...
            >
              <div>
                <label className="block text-xs uppercase tracking-wide text-charcoal-soft/60 mb-2">{t.contact.name}</label>
                <input
                  required
                  value={form.name}
                  onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                  className="w-full border border-hairline px-4 py-3 text-sm focus:outline-none focus:border-gold bg-transparent"
                />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wide text-charcoal-soft/60 mb-2">{t.lead.email}</label>
                <input
                  required
                  type="email"
                  value={form.email}
              ...
              <div>
                <label className="block text-xs uppercase tracking-wide text-charcoal-soft/60 mb-2">{t.lead.phone}</label>
                <input
                  value={form.phone}
              ...
              <div>
                <label className="block text-xs uppercase tracking-wide text-charcoal-soft/60 mb-2">{t.contact.message}</label>
                <textarea
                  required
                  rows={4}
                  value={form.message}
                  onChange={(e) => setForm((f) => ({ ...f, message: e.target.value }))}
                  className="w-full border border-hairline px-4 py-3 text-sm focus:outline-none focus:border-gold bg-transparent resize-none"
                />
              </div>
